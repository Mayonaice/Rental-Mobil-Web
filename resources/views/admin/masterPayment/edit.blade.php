        ...
        <ol class = "inline-flex items-center space-x-1 md:space-x-3">
            <li class = "inline-flex items-center">
                <a href="#"
                    ...
                    Payment > Edit Metode Payment
                </a>
            </li>
        </ol>
    </nav><br>
    <h1 class="text-4xl font-bold text-purple-600 max-w-md mx-auto mb-4">Edit Payment</h1>


    <div class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-lg">

        <form action="{{ route('masterPayment.update', $masterPayment) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">Nama Akun</label>
                <input type="text" id="name" name="nama_akun" value="{{$masterPayment->nama_akun}}"
            ...
            <div class="mb-4">
                ...
                <select id="selector" name="tipe_payment"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    required>
                        <option value="BCA" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'BCA' ? 'selected' : '' }}>BCA</option>
                        <option value="BNI" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'BNI' ? 'selected' : '' }}>BNI</option>
                        <option value="BRI" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'BRI' ? 'selected' : '' }}>BRI</option>
                        <option value="Permata" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'Permata' ? 'selected' : '' }}>Permata</option>
                        <option value="GoPay" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'Gopay' ? 'selected' : '' }}>Gopay</option>
                        <option value="Dana" {{ old('tipe_payment', $masterPayment->tipe_payment) == 'Dana' ? 'selected' : '' }}>Dana</option>
                </select>
            </div>

            <div id="bank" style="display: block;" class="mb-4">
                <label for="number" class=" text-sm font-medium text-gray-700">No Rekening</label>
                <input type="text" name="no_rek" value="{{$masterPayment->no_rek}}"
            ...
            <div id="nohp" style="display: none;" class="mb-4">
                ...
                <input type="text" name="no_hp" value="{{$masterPayment->no_hp}}"
                    class="mt-1  w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                    placeholder="No Hp"  />
            </div>
